...
#+begin_src R
  oxmap <- createBufferMap(geo = sf::st_geometry(oxdata$geo$predict_map), voronoi = TRUE, buffer = TRUE, dist = 18100, coast = TRUE, coastline = sf::st_union(oxdata$geo$gov_map))
#+end_src
...
#+begin_src R
  clust <- geostclust(geo = sf::st_geometry(oxdata$geo$predict_map),
                      times = as.POSIXct(paste0(gsub("_", "", colnames(oxdata$data)[720:1463]), ":00:00")),
                      cuts = c(4, 8, 16, 32),
                      method = "hstdbscan",
                      eps1 = 18100, eps2 = 3600, minPts = 6,
                      vals = list(list(D = ifelse(is.na(oxdata$data[, 720:1463]), -999, oxdata$data[, 720:1463]), #oxdata$data,
                                       delta_eps = 20)))
#+end_src
